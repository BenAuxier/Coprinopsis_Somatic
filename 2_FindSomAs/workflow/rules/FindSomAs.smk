...
# 	shell:
# 		"cp {path2hmmers}/{wildcards.prot}_hits/{wildcards.prot}_hits*{wildcards.sample}* {output}"

rule prepare_genomes:
	output:
		"data/Genomes/{sample}.fa"
	shell:
		"ln -sf {path2genomes}/*{wildcards.sample}*/*.1/*{wildcards.sample}*fna {output}"

rule prepare_gffs:
	output:
		"data/Annotations/{sample}.gff"
	shell:
		"ln -sf {path2genomes}/*{wildcards.sample}*/*.1/genomic.gff {output}"
...
rule hmmer2bed:
	...
	params:
		flanks = FLANKs
	shell:
		"python {hmmer2bed} {input.gff} {input.hmmer} -f {params.flanks} > {output}" 
...
rule get_GFFSlicer:
	output:
		temp("workflow/scripts/GFFSlicer.py")
	shell:
...
rule pretty_gff:
	...
	run:
		...
		nachtids = [tab[6] for tab in tabs_nacht]
		kinaseids = [tab[6] for tab in tabs_kinase]
		...
		with open(input.gff, 'r') as file, open(output.gff, 'w') as ofile: # Start a gff to put all the genes
			for line in file:
				if ('\tgene\t' in line) or ('\tmRNA\t' in line): # Only modify the genes and mRNA features
					...
					for att in attributes:
						if 'locus_tag=' in att:
							locus_tag = att.replace('locus_tag=', '')
...
rule report_counts:
	...
	output:
		"results/Loci_counts_GenBank.txt"
	shell:
...
rule get_gffutils2fasta:
	output:
		temp("workflow/scripts/gffutils2fasta.py")
	shell:
...
rule make_prot_fastas:
	...
	run:
		...
		if len(NACHTs) > 0:
			...
			cmd = f"python {input.gffutils2fasta} {input.genome} {input.gff} --type CDS --join -p --specificgene {NACHTs} --onlynames --output {output.nachts}"
			shell(cmd)
			shell(f"sed -i 's/>/>{wildcards.sample}_/' {output.nachts}")

			cmd = f"python {input.gffutils2fasta} {input.genome} {input.gff} --type CDS --join -p --specificgene {Kinases} --onlynames --output {output.kinases}"
			shell(cmd)
